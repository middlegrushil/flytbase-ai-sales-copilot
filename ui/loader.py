...
def load_outputs():
    ...
    return {
        ...
        "case_study": load_json(
            "output/case_study.json"
        ),

        "partner_recommendation": load_markdown(
            "output/partner_recommendation.md"
        ),

        "meeting_prep": load_markdown(
            "output/meeting_prep.md"
        ),

        "followup_email": load_markdown(
            "output/followup_email.md"
        ),

        "discovery_questions": load_markdown(
            "output/discovery_questions.md"
        ),

        "objections": load_json(
        ...
        "stakeholders": load_json(
            "output/stakeholders.json"
        ),

        "crm_summary": load_markdown(
            "output/crm_summary.md"
        ),

        "next_action": load_json(
            "output/next_action.json"
        ),

        "sales_brief": load_markdown(
